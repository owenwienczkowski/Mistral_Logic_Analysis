...
df = read.csv("C:/Users/wxyzo/OneDrive/Desktop/Career/Code/Dissertation/Analysis/LLM_Logic_Results.csv")
...
# Analysis 3: Model x Prompt Accuracy Performance
...
# Analysis 6: Performance by Question Type
...
# Analysis 7: Reasoning Performance by Ground Truth (ReAct Only)
...
# Analysis 8: Reasoning Performance by Question Type (ReAct Only)
...
# Test 2: For reasoning prompts, is the type of success (fully correct vs. lucky guess)
# dependent on the prompt technique (CoT vs. ReAct)?
reasoning_outcomes <- reasoning_df[reasoning_df$correctness_flag == 1, ] # Only look at correct answers
contingency_table_reasoning <- table(reasoning_outcomes$prompt_technique, reasoning_outcomes$fully_correct_flag) # columns are 0 (lucky guess) and 1 (fully correct)

reasoning_test <- chisq.test(contingency_table_reasoning)
print("Chi-Squared Test for Outcome Type (Fully Correct vs. Lucky Guess) by Technique")
print(reasoning_test)

# Test 3: Is there a significant difference in reasoning accuracy between LLMs for the ReAct prompt?
react_df <- new_df[new_df$prompt_technique == "ReAct", ]
...
# Test 4: Is there a significant difference in reasoning accuracy between prompt techniques?
reasoning_df <- new_df[new_df$prompt_technique %in% c("CoT", "ReAct"), ]
reasoning_accuracy_test_all <- chisq.test(table(reasoning_df$prompt_technique, reasoning_df$reasoning_correct_flag))
print("Chi-Squared Test for Prompt Technique vs. Reasoning Accuracy (CoT vs. ReAct)")
print(reasoning_accuracy_test_all)
...
ggplot(reasoning_by_truth, 
  ...
  geom_col(position = position_dodge()) +
  scale_fill_brewer(palette = "Set1") +
  labs(title = "Reasoning Accuracy (ReAct Only) by Ground Truth",
...
ggplot(reasoning_by_q_type, 
  ...
  # Manually set the four colors for the prompt techniques to be colour-blind friendly
  scale_fill_brewer(palette = "Set1") +
  labs(title = "Reasoning Accuracy (ReAct Only) by Question Type",
       x = "Question Type",
       y = "Reasoning Accuracy",
       fill = "LLM") +
  theme_minimal()
